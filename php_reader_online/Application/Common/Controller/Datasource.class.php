...
class Datasource {
    ...
    public static function getRedis($config_name = NULL, $server_region = 'default',$sentinel='') {
        ...
        //-------------
        $config = C('rediscache');
        $redis_config = $config[$config_name];
        //有sentinel时
        if ($config_name == 'instance1') {

            if ($sentinel) {
                $sentinel_config = [['ip' => '127.0.0.1', 'port' => '26379'], ['ip' => '127.0.0.1', 'port' => '26380'], ['ip' => '127.0.0.1', 'port' => '26381']];

                for ($i = 0; $i < 3; $i++) {
                    try {
                        $sentinel = new \Redis();
                        $sentinel->connect($sentinel_config[$i]['ip'], $sentinel_config[$i]['port']);
                        $res= $sentinel->rawCommand('SENTINEL', 'get-master-addr-by-name','mymaster');
                        $redis_config[$server_region]['host']=$res[0];
                        $redis_config[$server_region]['port']=$res[1];
                        break;
                    } catch (\Exception $e) {

                        \Think\Log::write($e->getMessage(),'bySelf-fatal-error');
                        die(json_encode(['errorno'=>'500100','msg'=>'网络异常','data'=>[]],JSON_UNESCAPED_UNICODE));
        ...
        try {
            ...
            \Think\Log::write($e->getMessage(),'bySelf-fatal_error');
            die(json_encode(['errorno'=>'500100','msg'=>'网络异常','data'=>[]],JSON_UNESCAPED_UNICODE));
        }
        return self::$redises[$config_name];
    }
...
class RedisHelper {
    ...
    private function __construct($config_name, $redis_config, $server_region) {
        ...
        if ($config_name && $redis_config && $server_region) {
            ...
            $this->_redis = null;
            \Think\Log::write('redis:'.$config_name.'may be down','bySelf-fatal_error');
            die(json_encode(['errorno'=>'500100','msg'=>'网络异常','data'=>[]],JSON_UNESCAPED_UNICODE));
...
class sset{
    ...
    public function __call($name,$vlue){
        return '';
    }
}

...
@interface ZZPhotoBrowerViewController ()<UICollectionViewDelegate,UICollectionViewDataSource,UIScrollViewDelegate>
...
-(void)backItemMethod
{
    self.passSelectPhotoArrayBlock(self.selectedPhotoArray);
...
-(void) makeCollectionViewUI
{
    ...
    flowLayout.itemSize = (CGSize){self.view.frame.size.width,self.view.frame.size.height-NavBarHeight};
...
- (void) viewDidLoad {
    ...
    self.navigationItem.leftBarButtonItem = [UIBarButtonItem itemWithTarget:self action:@selector(backItemMethod) image:Back_Btn_Pic];;
    
    
    //------------------
    if ([_enterFromPreview isEqualToString:@"1"]) {
        _rightBtn = [UIButton buttonWithType:UIButtonTypeCustom];
        _rightBtn.frame = CGRectMake(0, 0, 30, 30);

		 [_rightBtn setImage:[UIImage imageNamed:@"picture_unselected"] forState:UIControlStateNormal];
//        [_rightBtn setBackgroundImage:[UIImage imageNamed:@"picture_unselected"] forState:UIControlStateNormal];
        [_rightBtn setImage:[UIImage imageNamed:@"forward_resumes_blue_btn"] forState:UIControlStateSelected];
        [_rightBtn addTarget:self action:@selector(selectBtnClick:) forControlEvents:UIControlEventTouchUpInside];
        _rightBarButtonItem = [[UIBarButtonItem alloc] initWithCustomView:_rightBtn];
        
        //-----------
        UIBarButtonItem *negativeSpacer = [[UIBarButtonItem alloc]initWithBarButtonSystemItem:UIBarButtonSystemItemFixedSpace target:nil action:nil];
        negativeSpacer.width = 6;
        self.navigationItem.rightBarButtonItems = [NSArray arrayWithObjects:negativeSpacer, _rightBarButtonItem, nil];
        
    }else if ([_enterFromPreview isEqualToString:@"2"]){//预览按钮
    }
//    _rightBtn = [UIButton buttonWithType:UIButtonTypeCustom];
//    _rightBtn.frame = CGRectMake(0, 0, 25, 25);
//    [_rightBtn setBackgroundImage:[UIImage imageNamed:@"photoLibrary_unselect"] forState:UIControlStateNormal];
//    [_rightBtn setBackgroundImage:[UIImage imageNamed:@"forward_resumes_select"] forState:UIControlStateSelected];
//    [_rightBtn addTarget:self action:@selector(selectBtnClick:) forControlEvents:UIControlEventTouchUpInside];
//    _rightBarButtonItem = [[UIBarButtonItem alloc] initWithCustomView:_rightBtn];
//
//    //-----------
...
- (void)selectBtnClick:(UIButton *)sender {
    ...
    if (sender.isSelected) {
        if (self.selectedPhotoArray.count > 0) {
            ...
            sender.selected = NO;
            ZZPhoto *photo = self.photoData[currentIndex];
            photo.isSelect = NO;
            if (self.selectedPhotoArray.count > 0) {
                [self.selectedPhotoArray removeObject:photo];
            }
        }

    }else {
        if (self.selectedPhotoArray.count < self.maxSelectNumber) {

            ZZPhoto *photo = self.photoData[currentIndex];
            photo.isSelect = YES;
            sender.selected = YES;
            [self.selectedPhotoArray addObject:photo];
        }else {
            [ZJPAlert showAlertWithMessage:[NSString stringWithFormat:@"最多选择%ld张",self.maxSelectNumber] time:2.0f];
        }
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    currentIndex = (offsetx + width  * 0.5) / width ;
    if (currentIndex > self.photoData.count - 1) {
        currentIndex = self.photoData.count - 1;
    }
    if (currentIndex < 0) {
        currentIndex = 0;
    }
    
    ZZPhoto *photo = [_photoData objectAtIndex:currentIndex];
    if (photo.isSelect) {
        _rightBtn.selected = YES;
    }else {
        _rightBtn.selected = NO;

...
- (void)setImagePositionWithType:(SSImagePositionType)type spacing:(CGFloat)spacing {
    ...
    switch (type) {
        ...
        case SSImagePositionTypeTop: {
            ...
            //  below the image
            self.titleEdgeInsets = UIEdgeInsetsMake(8, - imageSize.width, - (imageSize.height + spacing), 0);
            
            // raise the image and push it right so it appears centered
            //  above the text
            self.imageEdgeInsets = UIEdgeInsetsMake(- (titleSize.height + spacing - 8), 0, 0, - titleSize.width);
            break;
        }

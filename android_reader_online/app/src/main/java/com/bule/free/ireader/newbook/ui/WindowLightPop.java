...
@Deprecated
public class WindowLightPop extends PopupWindow implements ReadBookConfig.ModeChangeable {
    ...
    public void setScreenBrightness() {
        if (ReadBookConfig.INSTANCE.getUiMode().isNight()) return;
        WindowManager.LayoutParams params = ((Activity) mContext).getWindow().getAttributes();
    ...
    public void changeMode(@NotNull ReadBookConfig.UIMode uiMode) {
        view.findViewById(R.id.ll_light_menu_bg).setBackgroundColor(uiMode.getMenuBarBgColor());
        if (ReadBookConfig.INSTANCE.getUiMode().isNight()) {
            scbFollowSys.setChecked(false);
            scbFollowSys.setEnabled(false);
        } else {
            scbFollowSys.setChecked(true);
            scbFollowSys.setEnabled(true);
        }
    }

...
/**
 * jake wharton 大佬的kotter knife框架，就一个文件，kotlin版butter knife，安装as插件食用更佳
 */

// 自定义部分
fun <V : View> ViewHolderImpl<*>.bindView(id: Int): ReadOnlyProperty<ViewHolderImpl<*>, V> = required(id, viewFinder)

private val ViewHolderImpl<*>.viewFinder: ViewHolderImpl<*>.(Int) -> View? get() = { itemView.findViewById(it) }
// 自定义部分 end
...
private val DialogFragment.viewFinder: DialogFragment.(Int) -> View?
    get() = {
        dialog?.findViewById(it) ?: view?.findViewById(it)
    }

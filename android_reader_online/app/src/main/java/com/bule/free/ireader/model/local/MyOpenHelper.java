...
public class MyOpenHelper extends SQLiteOpenHelper implements IOpenHelper {
    ...
    public MyOpenHelper(Context context, String name) {
        super(context, name, null, VERSION_3_3_4);
    }
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        if (oldVersion == VERSION_3_3_2 || oldVersion == VERSION_3_3_3) {
            String sql1 = " ALTER TABLE " + getTableName(BookContentBean.class) + " ADD COLUMN bookId TEXT DEFAULT ''";
            String sql2 = " ALTER TABLE " + getTableName(DownloadTaskBean2.class) + " ADD COLUMN currentBookCover TEXT DEFAULT ''," +
                    " currentChapterTitle TEXT DEFAULT '', currentBookTitle TEXT DEFAULT '', isChecked INTEGER  DEFAULT 0";
            try {
                db.execSQL(sql1);
            } catch (Exception e) {
                e.printStackTrace();
    ...
    public void update(Object obj, String id) {
        ...
        createTableIfNotExists(table);
        //具体实现保存数据方法
        update(obj, id, table, getWritableDatabase());
    }
     ...
     * @param db    操作数据库
     */
    private void update(Object obj, String id, Class<?> table, SQLiteDatabase db) {
        //将一个对象中的所有字段添加到该数据集中
        ContentValues contentValues = new ContentValues();
        //通过反射获取一个类中的所有属性
        Field[] declaredFields = table.getDeclaredFields();
        ...
        //对于一般的数据操作，我们采用通常是insert来插入数据，但是为了防止同一个对象的数据进行刷新，所以采用直接替换掉
        db.update(table.getName().replaceAll("\\.", "_"), contentValues, "_id=?", new String[]{id});
    }
    ...
    @Nullable
    @Override
    public synchronized <T> List<T> queryAll(Class<T> table, String orderBy) {
    ...
    @Nullable
    @Override
    public synchronized <T> List<T> queryAll(Class<T> table, String orderBy, int limit) {
        //如上雷同，不做介绍
        List<T> result = new ArrayList<>();
        if (!isTableExists(table)) {
            return result;
     ...
     * @param table     类类型
     * @param orderBy   指定顺序
     * @param selection 指定的条件
     * @param <T>
     * @return
     */
    @Nullable
    @Override
    public synchronized <T> List<T> queryByWhere(Class<T> table, String selection, String[] selectionArgs, String orderBy) {
        //如上雷同，不做介绍
        List<T> result = new ArrayList<>();
        if (!isTableExists(table)) {
            return result;
        ...
        SQLiteDatabase db = getReadableDatabase();
        Cursor cursor = db.query(table.getName().replaceAll("\\.", "_"), null, selection, selectionArgs, null, null, orderBy, null);
        result = initList(table, cursor);
    ...
    @Nullable
    @Override
    public synchronized <T> T queryById(Class<T> table, Object id) {
        if (!isTableExists(table)) {
            return null;
        }
        Field idField = null;
    ...
    /**
     * 通过一个字段名条件删除，通过字段条件删除的row都要将那个字段的命名固定，因为用的是反射
     *
     * @param clz    通过类名获取表
     * @param column 一个property的字面name
     * @param value  条件删除的判断条件的property的值
     */
    @Override
    public int deleteByWhere(Class<?> clz, String column, String value) {
        //首先获取该类中的id，如果有就会获取到
        SQLiteDatabase db = getWritableDatabase();
        //如果不存在属性id，就不需要删除
        return db.delete(clz.getName().replaceAll("\\.", "_"),
                column + " = ?",
                new String[]{value});
    }
    ...
    private <T> List<T> initList(Class<T> table, Cursor cursor) {
        ...
        while (cursor.moveToNext()) {
            try {
                ...
                LogUtils.e(String.format("cursor 转换数组抛出异常: %s", e));
                e.printStackTrace();
            }
        }
        return result;
    ...
    private boolean isTableExists(Class table) {
        ...
        Cursor cursor = db.query("sqlite_master",
                null,
                "type = 'table' and name = ?",
                new String[]{table.getName().replaceAll("\\.", "_")},
                null, null, null);
...
//            builder.append("id Integer PRIMARY KEY AUTOINCREMENT,");
    ...
    @Nullable
    private Field getFieldByColumnName(Class table, String columnName) {
        Field fieldId = null;
        try {
            fieldId = table.getDeclaredField(columnName);
        } catch (NoSuchFieldException e) {
        }
        return fieldId;
    }
